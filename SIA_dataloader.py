...
def get_subjects(config):
    ...
    for i, (source, gt, dark) in track(
        ...
        if "predict" in config.job_name:
            subject = tio.Subject(source=tio.ScalarImage(source), gt=tio.LabelMap(gt),source_path=str(source))
        else:
...
class SIADataLoader(torch.utils.data.Dataset):
            ...
            if "SAM" in config.module_list:
                training_transform = Compose(
                    [
                        ...
                        RandomSwap(
                            patch_size=self.swap_size, num_iterations=config.swap_iterations, include=["spatial_source"]#include="spatial_source"
                        ),
                        ZNormalization(),
...
def main(config):
    ...
    dataset = SIADataLoader(config=config)
