...
def predict(model, config, logger):
    ...
    dataset = SIADataLoader(config).subjects  # ! notice in predict.py should use Dataset(conf).subjects
    ...
    for i, item in enumerate(dataset):
        ...
        with torch.no_grad():
            for j, batch in enumerate(patch_loader):
                ...
                # gt_back[gt_ == 0] = 1
                # gt = torch.cat([gt_back, gt_], dim=1) 
            ...
            # * save pred mhd file
            save_mhd(pred_t, affine, i, config,original_path)
...
def save_mhd(pred, affine, index, config, original_filename):
    save_base = os.path.join(config.hydra_path, "pred_file")
    os.makedirs(save_base, exist_ok=True)
    base_name = os.path.basename(original_filename).split('.')[0]
    output_filename = f"{base_name}_pred.nii"
    pred_data = tio.ScalarImage(tensor=pred, affine=affine)
    pred_data.save(os.path.join(save_base, output_filename))
...
def main(config):
    ...
    if config.network == "res_unet":
        ...
        # from models.three_d.SIA_Unet_mt_no_SAM import SIAUNet as SIA_BETA
        # from models.three_d.SIA_Unet_mt_no_IAM import SIAUNet as SIA_BETA
        model = SIA_BETA(
        ...
        model = SIA_VNET(in_channels=config.in_classes, classes=config.out_classes, module_list=config.module_list)
