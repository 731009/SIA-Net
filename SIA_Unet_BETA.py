        ...
        if self.training:
            ...
            if "IAM" in self.module_list:
                with torch.no_grad():
                    ...
                    intensity_out = self.convx1(intensity_out)
            ...
            with torch.no_grad():
                out_pre = self.convx2(self.convx1(out))

            out = self.convx3(out_pre)
...
if __name__ == "__main__":
    ...
    model = SIAUNet(in_channels=1, out_channels=2, init_features=32)
    model.train()
    y, s_y, i_y = model(x,x,x)
    mse_loss = nn.MSELoss()
    # print(y.size())
    # print(i_y)
